...
import { CreateCategoryApi, GetCategoriesApi, DeleteCategoryApi } from '@/apiEndpoints/category';
...
const ManageCategory = () => {
    ...
    const [editModalCategoryData, setEditModalCategoryData] = useState<CategoryResponse | null>(null);
    const [editModalCategory, setEditModalCategory] = useState<string | null>(null);
    ...
    const openEditModal = (categoryId: string) => {
        setEditModalCategory(categoryId);
    };
    ...
    useEffect(() => {
        ...
        dispatch(
            saveCategoryLength({ categoryLength: categories.length || 0 })
        );
    }, [categoryLength]);
    ...
    return (
        <div className='manage-category-container'>
            <div className='manage-category-header'>
                ...
                <button className='create-category-button' onClick={openModal}>Create Category</button>
            </div>
                ...
                {categories && categories.map((category) => (
                    <div key={category._id} className='manage-category-card'>
                        <div className='manage-category-image-title'>
                            <img src={`http://localhost:3000/uploads/${category.file}`} className='manage-category-image' />
                            <p className='manage-category-name'>{category.categoryName}</p>
                        </div>
                        <div className='manage-category-button'>
                            <RiFileEditFill className='edit-button' onClick={() => openEditModal(category._id)} />
                            <Modal
                                isOpen={editModalCategory === category._id}
                            ...
                            <Modal
                                ...
                                <div>
                                    ...
                                    <p>Are you sure you want to delete this category? All the Items under this cateory will be deleted too.</p>
                                    <div className='delete-modal-button'>
                                        ...
                                        <button className='no-button' onClick={closeDeleteModal}>No</button>
                                    </div>
                                </div>
                                <CgCloseR className='close-button' onClick={closeDeleteModal} />
                            </Modal>
                            <AiFillDelete className='delete-button' onClick={() => { openDeleteModal() }} />
                        </div>
                    </div>
                ))}
            </div>
