...
export const CreateItemsApi = (data: any) => {
  return axiosInstanceToken
    .post("/api/v1/item/create-item", data)
    .then((response) => {
...
export const GetAllItemsApi = async (
  ...
  try {
    ...
    const response = await axiosInstance(
      `/api/v1/item/get-all-items?${queryString}`
    );
...
export const GetItemsByCategoryIDApi = async (id: string) => {
  try {
    const response = await axiosInstance(
      `/api/v1/item/get-items-by-category/${id}`
    );
    const data = response.data;
...
export const GetItemByIdApi = async (id: string) => {
  try {
    const response = await axiosInstance(`/api/v1/item/get-item-by-id/${id}`);
    const data = response.data;
...
export const UploadImageToItemApi = (id: string, data: any) => {
  ...
  return axiosInstanceToken
    .post(`/api/v1/item/upload-image/${id}`, formData)
    .then((response) => {
      console.log("response", response.data.data);
      // toast.success(response.data.message);
      return response.data.data;
...
export const RemoveImageFromItemApi = (id: string, filename: string) => {
  return axiosInstanceToken
    .delete(`/api/v1/item/remove-images-from-item/${id}/${filename}`)
    .then((response) => {
      console.log("response", response.data.data);
      toast.success(response.data.message);
      return response.data.data;
...
export const UpdateItemApi = (id: string, data: any) => {
  return axiosInstanceToken
    .patch(`/api/v1/item/update-item/${id}`, data)
    .then((response) => {
      console.log("response", response.data.data);
      toast.success(response.data.message);
      return response.data.data;
...
export const CategoryItemCountApi = (id: string) => {
  return axiosInstanceToken
    .get(`/api/v1/item/get-item-count/${id}`)
    .then((response) => {
      console.log("response", response.data.data);
      return response.data.data;
    })
